...
def MLLGfuncsparse(T, N, tau, h, m, rho, L, tolgmres, eps, mu, sig, theta, alpha, Ce):  # ,J,m0,E0,H0):
    ...
    class MyExpressionH(UserExpression):
        def eval(self, value, x):
            ...
            s3s=s3**2
            value[0] = 0
            value[1] =-2*np.pi*s1s*s2s*s3*cos(np.pi*x[2])  /mu
            value[2] = 2*np.pi*s1s*s2*cos(np.pi*x[1])*s3s   /mu
        def value_shape(self):
            return (3,)    
    
    
    (Lhs,rhsLLGH, M0,mj,Ej,Hj,JccE,JE,nBEM,nFEM,nMAG,X,trace_space,VV,V3,wei,boundary_rhs,dtmfunc)=assLHS( tau, h, m, eps, mu, sig,rho,L,N)
    mj=interpolate(Expression(exSolutionCode,multF=3,tend=10*T,t=0.0,degree=1),V3)
    Hex=interpolate(MyExpressionH(degree=1), X)
    ...
    for j in range(0, int(N)):  # time stepping: update from timestep t_j to j+1
        ...
        rhs1 = (-Ce * inner(nabla_grad(mj), nabla_grad(phi)) )*dx
        
        if (refadd<1.0):
            rhs1 = rhs1 +(inner(alpha*dtMex + cross(Mex,dtMex), phi) +Ce*inner(nabla_grad(Mex),nabla_grad(phi)))*dx
        if (onof >0.0 ):
            #lhs1= lhs1
            rhs1 = rhs1 +(onof*inner(Hj, phi) - onof*(tau*(j))**3/3*inner(Hex, phi))*dx
        ...
        Rhs = np.concatenate([eps / tau * M0 * Ej.vector() +1.0/3.0*t**3*M0.dot(JccE.vector())+t*(sig*t+2.0)*M0.dot(JE.vector()),#- ((j+1)*tau )**2 * M0 * J.vector(),
                              mu / tau * M0 * Hj.vector() - onof * mu * rhsLLGH.transpose() * (v.vector()-dtMex.vector()),
...
def assLHS( tau, h, m, eps, mu, sig,rho,L,N):
    ...
    (Lhs,M0,rhsLLGH,boundary_rhs,dtmfunc)=assLHS2(X,V3,mu,trace_matrix,dlt,m,trace_space,eps,tau,sig,nFEM,nBEM)

    stop = timeit.default_timer()


    wei= assCQw(L,tau,m,mu,eps,trace_space.grid,dlt,rho,N,stop,start)
...
def initialdata(X,V3):
    ...
    JE=interpolate(JE, X)#project(Expression(['0.00','000.0','00.0'],degree=1),X)
...
def assLHS2(X,V3,mu,trace_matrix,dlt,m,trace_space,eps,tau,sig,nFEM,nBEM):
    ...
    Lhs = BlockedDiscreteOperator(np.array(blocke1))
